...
export const SandhyaPlayerMachine = setup({
  ...
  actors: {
    vimeoPlayerActor: fromCallback(({sendBack, receive, input}: {sendBack: any, receive: any, input: {container: HTMLDivElement}}) => {
      ...
      receive((event: any) => {
        if (event.type === 'LOAD_VIDEO') {
          ...
          player.on('loaded', () => {
            ...
            player!.setPlaybackRate(event.playbackSpeed).then(() => {
  ...
  guards: {
    ...
    isFirstRepeatPlay: (context) => {
      return context.context.mode === 'repeat' && 
             !context.context.lessons[context.context.currentLessonIndex].isLoopedLesson &&
  ...
  actions: {
    updateTimeRemaining: assign({
      timeElapsed: ({ context }) => {
        ...
        const completedLessonsDuration = context.lessons
          .slice(0, context.currentLessonIndex)
          .reduce((acc, lesson) => {
      ...
      on: {
        ...
        NEXT_LESSON: {
          ...
          actions: [
            // sendTo('vimeoPlayerActor', { type: 'PAUSE' }),
            assign({
              currentLessonIndex: ({ context }) => 
                context.currentLessonIndex + 1
            })
          ],
          guard: 'hasNextLesson'
        },
