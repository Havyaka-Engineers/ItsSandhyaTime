  ...
  return (
    <Page className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div ref={containerRef} className="h-[78%] bg-gray-900 overflow-hidden "></div>
        ...
        <div className="h-[22%] bg-white">
          ...
          <div className="flex-1 flex justify-center items-center">
            <span className="text-xl font-medium font-bold">
            ...
            <QueueListIcon className="w-5 h-5 text-gray-600 ml-2 cursor-pointer" onClick={() => setLessonListOpen(true)} />
          </div>
          {/* Row 4: Icons */}
          <div className="w-full flex justify-between items-center px-4 py-4">
            ...
            {isPlaying ? (
              <PauseCircleIcon className="w-16 h-16 text-red-500 cursor-pointer" onClick={() => send({ type: 'PAUSE' })} />
            ) : (
              <PlayCircleIcon className="w-16 h-16 text-red-500 cursor-pointer" onClick={() => send({ type: 'RESUME' })} />
            )}
            <ForwardIcon className="w-8 h-8 text-red-500" />
            <Cog6ToothIcon className="w-6 h-6 text-gray-600 cursor-pointer" onClick={handleSettingsClick} />
          </div>
        </div>
      </div>

      <LessonListModal opened={lessonListOpen} onClose={() => setLessonListOpen(false)} lessons={state.context.lessons} />
